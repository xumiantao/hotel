...
.pages span.current{
 ...
 <head> 
     ...
     <div class="ibox float-e-margins"> 
      <div class="ibox-title"> 
       ...
       <div class="btn-group" style="float:right;margin-top:-10px;">
          <button data-toggle="dropdown" class="btn btn-warning dropdown-toggle" onclick="location='/index.php/admin/product/productAdd'">添加产品+</button>
       </div>
      </div> 
       ...
       <div class="table-responsive"> 
        <table class="table table-striped"> 
         ...
         <?php if(is_array($product_list)): foreach($product_list as $key=>$item): ?><tr> 
           ...
           <td><?php echo ($item["categore"]); ?></td> 
           <td><?php echo ($item["integral"]); ?></td>
           <td><?php echo ($item["stock"]); ?></td> 
           <td><?php echo ($item["is_shelves"]); ?></td> 
           <td>
              <a href="/index.php/admin/product/productDetail?id=<?php echo ($item["id"]); ?>">查看</a>
              <a href="javascript:revoked(<?php echo ($item["id"]); ?>)">下架</a>
    ...
    <script type="text/javascript">
     function del(id){
    	 if (confirm('确定删除吗？') == true){
    		 $.ajax({
    			 url: '<?php echo U('Admin/product/ajax_product_delete');?>',
    			 type: 'post',
    			 dataType: 'json',
    		     data: {'id':id},
  ...
  <script type="text/javascript">
     function revoked(id){
    	 if (confirm('确定下架吗？') == true){
    		 $.ajax({
    			 url: '<?php echo U('Admin/product/ajax_product_revoked');?>',
    			 type: 'post',
    			 dataType: 'json',
    		     data: {'id':id},
    		     success: function (data) {
